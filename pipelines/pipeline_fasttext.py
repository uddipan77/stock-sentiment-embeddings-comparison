"""
Pipeline for FastText embeddings + Random Forest
"""
import sys
...
from src.embeddings.fasttext_embeddings import FastTextEmbeddings
...
logger = setup_logger('pipeline_fasttext', log_dir=RESULTS_DIR)

def run_fasttext_pipeline():
    """Run complete pipeline with FastText embeddings"""
    
    logger.info("="*80)
    logger.info("STARTING FASTTEXT + RANDOM FOREST PIPELINE")
    logger.info("="*80)
    ...
    # 3. Generate FastText Embeddings
    logger.info("\n" + "="*80)
    logger.info("STEP 3: GENERATING FASTTEXT EMBEDDINGS")
    logger.info("="*80)
    
    fasttext_embedder = FastTextEmbeddings()
    fasttext_embedder.train(train_tokens)
    fasttext_embedder.save_model('fasttext_model.bin')
    
    # Transform to embeddings
    X_train = fasttext_embedder.transform(train_tokens)
    X_val = fasttext_embedder.transform(val_tokens)
    X_test = fasttext_embedder.transform(test_tokens)
    ...
    trainer.save_model('random_forest_model.pkl', 'fasttext')
    ...
    evaluator.save_metrics('fasttext')
    ...
    visualizer.plot_confusion_matrix(cm, 
                                    title='FastText + RF Confusion Matrix',
                                    filename='fasttext_confusion_matrix.png')
    
    # Plot feature importance
    visualizer.plot_feature_importance(model, 
                                      filename='fasttext_feature_importance.png')
    
    logger.info("\n" + "="*80)
    logger.info("FASTTEXT PIPELINE COMPLETED SUCCESSFULLY")
    logger.info("="*80)
